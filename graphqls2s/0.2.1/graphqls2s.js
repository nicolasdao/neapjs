 ...
 * All rights reserved.
 * 
 * This source code is licensed under the BSD-style license (3-clause-BSD) 
...
/*eslint-disable */
var log = function log(msg, name) {
    if (name) console.log(name + ': ', msg);else console.log(msg);
    return msg;
};
/*eslint-enable */
...
var matchLeftNonGreedy = function matchLeftNonGreedy(str, startChar, endChar) {
    return chain(str.match(new RegExp(startChar + '(.*?)' + endChar))).next(function (m) {
        return m && m.length > 0 ? chain(matchLeftNonGreedy('' + m[m.length - 1] + endChar, startChar, endChar)).next(function (v) {
            return v ? v : m;
        }).val() : m;
    }).val();
};
...
module.exports = {
    ...
    matchLeftNonGreedy: matchLeftNonGreedy
};
...
var getSchemaBits = function getSchemaBits(sch) {
	return _.flatten(_.toArray(_([typeRegex, inputRegex, enumRegex, interfaceRegex, abstractRegex]).map(function (rx) {
		return _.toArray(_(escapeGraphQlSchemaPlus(sch, carrReturnEsc, tabEsc).match(rx)).map(function (str) {
			...
			if (!blockMatch) {
				var msg = 'Schema error: Missing block';log(msg);throw new Error(msg);
			}
			...
			var property = str.split(carrReturnEsc).join(' ').split(tabEsc).join(' ').replace(/ +(?= )/g, '');
			return { property: property, block: block };
		}));
	})));
};

var getSchemaEntity = function getSchemaEntity(firstLine) {
	return firstLine.indexOf('type') == 0 ? { type: 'TYPE', name: firstLine.replace('type', '').replace(/ /g, '').replace('{', '') } : firstLine.indexOf('enum') == 0 ? { type: 'ENUM', name: firstLine.replace('enum', '').replace(/ /g, '').replace('{', '') } : firstLine.indexOf('input') == 0 ? { type: 'INPUT', name: firstLine.replace('input', '').replace(/ /g, '').replace('{', '') } : firstLine.indexOf('interface') == 0 ? { type: 'INTERFACE', name: firstLine.replace('interface', '').replace(/ /g, '').replace('{', '') } : { type: null, name: null };
};

var getCommentsBits = function getCommentsBits(sch) {
	return _.toArray(_(escapeGraphQlSchemaPlus(sch, carrReturnEsc, tabEsc).match(/#(.*?)_cr_([^#]*?)({|:)/g) || []).filter(function (x) {
		return x.match(/{$/);
	}).map(function (c) {
		...
		var hashCount = parts.reduce(function (a, b) {
			return a + (b.indexOf('#') == 0 ? 1 : 0);
		}, 0);
		return { text: parts.initial(), property: getSchemaEntity(parts.last()), comments: hashCount == parts.size() - 1 };
	}).filter(function (x) {
		return x.comments;
	}).map(function (x) {
		return { text: x.text.join('\n'), property: x.property };
	}));
};

var genericNameAlias = function genericNameAlias(genName) {
	return chain(genName.match(genericTypeRegEx)).next(function (m) {
		return m ? chain(genName.split(m[0])).next(function (parts) {
			return '' + parts[0] + m[1].split(',').map(function (x) {
				return x.trim();
			}).join('');
		}).val() : genName;
	}).val
  ...
  */
	...
	return chain({ gType: genericType.trim(), gLetter: genericLetter.trim(), cType: concreteType.trim() }).next(function (_ref) {
		var gType = _ref.gType,
		    gLetter = _ref.gLetter,
		    cType = _ref.cType;
...
var getTypeDetails = function getTypeDetails(t) {
	return chain({ originName: t, isGen: t.indexOf('<') > 0, name: genericNameAlias(t) }).next(function (result) {
		if (result.isGen && !memoizedGenericSchemaObjects[result.name]) memoizedGenericSchemaObjects[result.name] = result;
		return result;
	}).val();
};

var getPropertyValue = function getPropertyValue(_ref2, mapResultName) {
	var name = _ref2.name,
	    params = _ref2.params,
	    result = _ref2.result;
	return '' + name + (params ? '(' + params + ')' : '') + (result && result.name ? ': ' + (mapResultName ? mapResultName(result.name) : result.name) : '');
};
 ...
 *         }]             									Property breakdown
 */
var getBlockProperties = function getBlockProperties(blockParts, baseObj, metadata) {
	return chain(_(metadata).filter(function (m) {
		...
		return _(blockParts).reduce(function (a, part) {
			...
			if (p.indexOf('#') == 0) a.comments.push(p);else {
				var prop = p.replace(/ +(?= )/g, '').replace(/,$/, '');
				var paramsMatch = prop.match(propertyParamsRegEx);
				var propDetails = paramsMatch ? chain(prop.split(paramsMatch[0])).next(function (parts) {
					return { name: parts[0].trim(), metadata: mData, params: paramsMatch[1], result: getTypeDetails((parts[1] || '').replace(':', '').trim()) };
				}).val() : chain(prop.split(':')).next(function (parts) {
					return { name: parts[0], metadata: mData, params: null, result: getTypeDetails((parts[1] || '').trim()) };
				}).val();
				a.props.push({
...
var getSchemaObject = function getSchemaObject(definitions, typeName, nameRegEx, metadata) {
	return _.toArray(_(definitions).filter(function (d) {
		...
		var typeMatch = name.match(genericTypeRegEx);
...
var getInterfaces = function getInterfaces(definitions, metadata) {
	return getSchemaObject(definitions, 'interface', interfaceNameRegEx, metadata);
};

var getAbstracts = function getAbstracts(definitions, metadata) {
	return getSchemaObject(definitions, 'abstract', abstractNameRegEx, metadata);
};

var getTypes = function getTypes(definitions, metadata) {
	return getSchemaObject(definitions, 'type', typeNameRegEx, metadata);
};

var getInputs = function getInputs(definitions, metadata) {
	return getSchemaObject(definitions, 'input', inputNameRegEx, metadata);
};

var getEnums = function getEnums(definitions, metadata) {
...
var getObjWithExtensions = function getObjWithExtensions(obj, schemaObjects) {
	if (obj && schemaObjects && obj.inherits) {
		...
		var superClassWithInheritance = getObjWithExtensions(superClass, schemaObjects);

		var objWithInheritance = {
			...
			implements: _.toArray(_.uniq(_.concat(obj.implements, superClassWithInheritance.implements).filter(function (x) {
				return x;
			}))),
	...
	} else return getObjWithInterfaces(obj);
};

var getObjWithInterfaces = function getObjWithInterfaces(obj, schemaObjects) {
	if (obj && schemaObjects && obj.implements && obj.implements.length > 0) {
		var interfaceWithAncestors = _.toArray(_.uniq(_.flatten(_.concat(obj.implements.map(function (i) {
			return getInterfaceWithAncestors(i, schemaObjects);
		})))));
		return {
			type: obj.type, name: obj.name, genericType: obj.genericType, originalBlockProps: obj.blockProps,
...
var memoizedInterfaceWithAncestors = {};
var getInterfaceWithAncestors = function getInterfaceWithAncestors(_interface, schemaObjects) {
	if (memoizedInterfaceWithAncestors[_interface]) return memoizedInterfaceWithAncestors[_interface];
	var interfaceObj = schemaObjects.filter(function (x) {
		return x.name == _interface;
	}).first();
	...
	var interfaceWithAncestors = interfaceObj.implements && interfaceObj.implements.length > 0 ? _.toArray(_.uniq(_.flatten(_.concat([_interface], interfaceObj.implements, interfaceObj.implements.map(function (i) {
		return getInterfaceWithAncestors(i, schemaObjects);
	}))))) : [_interface];
...
var addComments = function addComments(obj, comments) {
	obj.comments = _(comments).filter(function (c) {
		return c.property.type == obj.type && c.property.name == obj.name;
	}).map(function (x) {
		return x.text;
	}).first();
	return obj;
...
var parseSchemaObjToString = function parseSchemaObjToString(comments, type, name, _implements, blockProps) {
	return ['' + (comments && comments != '' ? '\n' + comments : ''), type.toLowerCase() + ' ' + name + (_implements && _implements.length > 0 ? ' implements ' + _implements.join(', ') : '') + ' { ', blockProps.map(function (prop) {
		return '    ' + (prop.comments != '' ? prop.comments + '\n    ' : '') + prop.value;
	}).join('\n'), '}'].join('\n');
};

var isTypeGeneric = function isTypeGeneric(type, genericLetter) {
	return !type || !genericLetter ? false : type == genericLetter ? true : type.indexOf('[') == 0 && type.indexOf(']') > 0 ? _(type.match(/\[(.*?)\]/)[1].split(',')).some(function (x) {
		return x.trim() == genericLetter;
	}) : type.indexOf('>') > 0 && type.indexOf('>') > 0 ? _(type.match(/<(.*?)>/)[1].split(',')).some(function (x) {
		return x.trim() == genericLetter;
	}) : false;
};

var memoizedNewSchemaObjectFromGeneric = {};
var createNewSchemaObjectFromGeneric = function createNewSchemaObjectFromGeneric(_ref3, schemaBreakDown) {
	var originName = _ref3.originName,
	    isGen = _ref3.isGen,
	    name = _ref3.name;

	if (isGen && !memoizedNewSchemaObjectFromGeneric[name]) {
		var genObjName = chain(originName.split('<')).next(function (parts) {
			return parts[0] + '<';
		}).val();
		var concreteType = (originName.match(/<(.*?)>/) || [null, null])[1];
		if (!concreteType) throw new Error('Schema error: Cannot find generic type in object ' + originName);
		var baseGenObj = schemaBreakDown.find(function (x) {
			return x.name.indexOf(genObjName) == 0;
		});
		...
		var blockProps = baseGenObj.blockProps.map(function (prop) {
			return isTypeGeneric(prop.details.result.name, baseGenObj.genericType) ? chain({ name: prop.details.name, params: prop.params, result: {
		...
		var newSchemaObjStr = parseSchemaObjToString(baseGenObj.comments, baseGenObj.type, name, baseGenObj.implements, blockProps);
		return {
...
var buildSchemaString = function buildSchemaString(schemaObjs) {
	return _(schemaObjs).filter(function (x) {
		return !x.genericType && x.type != 'ABSTRACT';
	}).map(function (obj) {
		return parseSchemaObjToString(obj.comments, obj.type, obj.name, obj.implements, obj.blockProps);
	}).join('\n') + _(memoizedGenericSchemaObjects).map(function (value) {
		return createNewSchemaObjectFromGeneric(value, schemaObjs).stringObj;
	}).join('\n');
};

var getSchemaParts = function getSchemaParts(graphQlSchema, metadata, includeNewGenTypes) {
	return chain(_([getInterfaces, getAbstracts, getTypes, getInputs, getEnums].reduce(function (objects, getObjects) {
		return objects.concat(getObjects(getSchemaBits(graphQlSchema), metadata));
	}, []))).next(function (firstSchemaBreakDown) {
		return _.toArray(firstSchemaBreakDown.map(function (obj) {
			return getObjWithExtensions(obj, firstSchemaBreakDown);
		}).map(function (obj) {
			return addComments(obj, getCommentsBits(graphQlSchema));
		}));
	}).next(function (v) {
		return includeNewGenTypes ? v.concat(_.toArray(_(memoizedGenericSchemaObjects).map(function (value) {
			return createNewSchemaObjectFromGeneric(value, v).obj;
		}))) : v;
	}).val();
};
...
var graphqls2s = {
	getSchemaAST: function getSchemaAST(graphQlSchema) {
		return chain(resetMemory()).next(function () {
			return { metadata: extractGraphMetadata(graphQlSchema), stdSchema: removeGraphMetadata(graphQlSchema) };
		}).next(function (metadata) {
			return getSchemaParts(metadata.stdSchema, metadata.metadata, true);
		}).next(function (v) {
			resetMemory();return v;
		}).val();
	},
	transpileSchema: function transpileSchema(graphQlSchema) {
		return chain(resetMemory()).next(function () {
			return buildSchemaString(getSchemaParts(removeGraphMetadata(graphQlSchema)));
		}).next(function (v) {
			resetMemory();return v;
		}).val();
	},
...
var extractGraphMetadata = function extractGraphMetadata() {
	...
	var escSchema = escapeGraphQlSchema(schema, carrReturnEsc, tabEsc).replace(/_t_/g, ' ');
	var attrMatches = escSchema.match(/@(.*?)(_cr_)(.*?)({|_cr_)/mg);
	var graphQlMetadata = chain(_(attrMatches).map(function (m) {
		return chain(m.split(carrReturnEsc)).next(function (parts) {
			...
			var typeMatch = (parts[0].trim() + ' ').match(/@(.*?)(\s|{|\(|\[)/);
			if (!typeMatch) {
				var msg = 'Schema error: Impossible to extract type from metadata attribute ' + parts[0];log(msg);throw new Error(msg);
			}
			var attrName = typeMatch[1].trim();
			var attrBody = parts[0].replace('@' + attrName, '').trim();
			...
			var _chain$next$val = chain(removeMultiSpaces(parts[1].trim())).next(function (t) {
				return t.match(/^(type\s|input\s|enum\s|interface\s)/) ? chain(t.split(' ')).next(function (bits) {
					return { schemaType: bits[0].toUpperCase(), value: bits[1].replace(/ /g, '').replace(/{$/, '') };
				}).val() : { schemaType: 'PROPERTY', value: t };
			}).val(),
			...
			var parent = schemaType == 'PROPERTY' ? chain(escSchema.split(m).join('___replace___')).next(function (s) {
				...
				var parentSchemaTypeName = m2[2].replace(/{/g, ' ').replace(/_cr_/g, ' ').trim().split(' ')[0];
		...
		return metadata.map(function (m) {
			return m.schemaType == 'PROPERTY' ? m.parent ? chain(metadata.find(function (x) {
				...
				return v ? function () {
					m.parent.metadata = { type: v.schemaType, name: v.name };return m;
				}() : m;
			}).val() : m : m;
		});
	}).next(function (metadata) {
		return _.toArray(metadata);
	}).val();
...
var removeGraphMetadata = function removeGraphMetadata() {
	var schema = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
	return escapeGraphQlSchema(schema, carrReturnEsc, tabEsc).replace(/_t_/g, ' ').replace(/@(.*?)_cr_/g, '').replace(/_cr_/g, '\n');
};
...
//# sourceMappingURL=graphqls2s.js.map